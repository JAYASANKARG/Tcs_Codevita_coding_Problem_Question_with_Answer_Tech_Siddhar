...
There are 20 samples and 3 ranges. The samples are 921, 107 195. The ranges are 1-100, 50-600 and 1-1000. Note that the ranges are overlapping. 
The number of samples in each of the three ranges are 1, 12 and 20 respectively. Hence the three lines of the output are 1, 12 and 20.
"""
def testrange(range1,range2,n):
    count=0
    for i in range(len(n)):
        if(range1<=n[i]<=range2):
            count+=1

    return count


size,testsize=map(int,input("Enter the size and range size : ").split(" "))
ppm=list(map(int,input("Enter the samples : ").split(" ")))

result=[]
for i in range(testsize):
    range1,range2=map(int,input("Enter the range : ").split(" "))
    result.append(str(testrange(range1,range2,ppm)))
